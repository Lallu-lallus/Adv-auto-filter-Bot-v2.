...
async def auto_filter(bot: Client, update: Message):
    ...
    if len(results) == 0: # double check
        ...
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"⭕ go to next Page 1/{len_result if len_result < max_pages else max_pages} ⭕", callback_data="ignore")
        ])
        ...
        try:
            await bot.send_message(
                ...
                text=f"⚜️check link at @cml_links⚜️.. Found {(len_results)} Results For Your request🤭😜: <code>{query}</code>",
